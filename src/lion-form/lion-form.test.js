 ...
 */
const fixture = /** @type {(arg: TemplateResult) => Promise<LionForm>} */ (
  _fixture
);
...
describe("<lion-form>", () => {
  ...
  it("works with the native submit event (triggered via a button)", async () => {
    ...
    button.click();
    viExpect(submitSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it("redispatches a submit event on the native form node when calling submit() imperatively", async () => {
    const nativeFormSubmitEventSpy = vi.fn();
    const el = await fixture(html`
      <lion-form>
    ...
    const submitGroupSpy = vi.spyOn(el, "submitGroup");
    el.submit();
    viExpect(submitSpy).toHaveBeenCalledTimes(1);
    viExpect(nativeFormSubmitEventSpy).toHaveBeenCalledTimes(1);
    viExpect(submitGroupSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it("handles internal submit handler before dispatch", async () => {
    ...
    const internalHandlerSpy = vi.spyOn(el, "submitGroup");
    const dispatchSpy = vi.spyOn(el, "dispatchEvent");
    await aTimeout(0);
    button.click();
    viExpect(internalHandlerSpy.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it("handles internal submit handler before dispatch", async () => {
    ...
    const dispatchSpy = vi.spyOn(el, "dispatchEvent");
    button.click();
    expect(dispatchSpy.mock.calls[0][0].type).to.equal("submit");
    viExpect(internalHandlerSpy.mock.invocationCallOrder[0]).toBeLessThan(
      dispatchSpy.mock.invocationCallOrder[0],
    );
  });
  ...
  it("handles internal reset handler before dispatch", async () => {
    ...
    const internalHandlerSpy = vi.spyOn(el, "resetGroup");
    const dispatchSpy = vi.spyOn(el, "dispatchEvent");
    button.click();
    expect(dispatchSpy.mock.calls[0][0].type).to.equal("reset");
    viExpect(internalHandlerSpy.mock.invocationCallOrder[0]).toBeLessThan(
      dispatchSpy.mock.invocationCallOrder[0],
    );
  });
