import { beforeEach, describe, expect as viExpect, it, vi } from "vitest";
...
describe("<lion-calendar>", () => {
  ...
  describe("Structure", () => {
    it("implements BEM structure", async () => {
      ...
      expect(el.shadowRoot?.querySelector(".calendar__previous-button")).to
        .exist;
      expect(el.shadowRoot?.querySelector(".calendar__next-button")).to.exist;
      expect(el.shadowRoot?.querySelector(".calendar__navigation-heading")).to
        .exist;
    ...
    it("has heading with month and year", async () => {
      vi.useFakeTimers();
      vi.setSystemTime(new Date("2000/12/01"));
    ...
    it("has previous year button", async () => {
      const el = await fixture(
        html`<lion-calendar
          .centralDate=${new Date("2019/11/20")}
        ></lion-calendar>`,
      );
      expect(el.shadowRoot?.querySelectorAll(".calendar__previous-button")[0])
        .dom.to.equal(`
    ...
    it("has next year button", async () => {
      ...
      expect(el.shadowRoot?.querySelectorAll(".calendar__next-button")[0]).dom
        .to.equal(`
    ...
    it("has previous month button", async () => {
      ...
      expect(el.shadowRoot?.querySelectorAll(".calendar__previous-button")[1])
        .dom.to.equal(`
    ...
    it("has next month button", async () => {
      ...
      expect(el.shadowRoot?.querySelectorAll(".calendar__next-button")[1]).dom
        .to.equal(`
  ...
  describe("Public API", () => {
    it('has property "centralDate" that determines currently visible month', async () => {
      const elObj = new CalendarObject(
        await fixture(html`
          <lion-calendar
            .centralDate="${new Date("2014/05/15")}"
          ></lion-calendar>
        `),
      );
    ...
    it('sets "centralDate" to today by default', async () => {
      vi.useFakeTimers();
      vi.setSystemTime(new Date("2013/03/15"));
    ...
    it('determines the date focusable with keyboard from "centralDate"', async () => {
      const elObj = new CalendarObject(
        await fixture(html`
          <lion-calendar
            .centralDate="${new Date("2017/07/05")}"
          ></lion-calendar>
        `),
      );
      expect(
        elObj.checkForAllDayObjs(
          /** @param {DayObject} o */ (o) =>
            o.buttonEl.getAttribute("tabindex") === "0",
      ...
      expect(
        elObj.checkForAllDayObjs(
          /** @param {DayObject} o */ (o) =>
            o.buttonEl.getAttribute("tabindex") === "-1",
          /** @param {number} n */ (n) => n !== 5,
        ),
      ).to.be.true;
    });
    ...
    it('has property "selectedDate" for the selected date', async () => {
      const el = await fixture(html`
        <lion-calendar
          .selectedDate="${new Date("2019/06/15")}"
        ></lion-calendar>
      `);
      const elObj = new CalendarObject(el);
    ...
    it('changes "centralDate" from default to "selectedDate" on first render if no other custom "centralDate" is provided', async () => {
      ...
      const el = await fixture(html`
        <lion-calendar
          .selectedDate="${new Date("2016/10/20")}"
        ></lion-calendar>
      `);
      const elObj = new CalendarObject(el);
    ...
    it('sends event "user-selected-date-changed" when user selects a date via mouse', async () => {
      ...
      `);
      const elObj = new CalendarObject(el);
      viExpect(dateChangedSpy).not.toHaveBeenCalled();
      elObj.getDayEl(15).click();
      await el.updateComplete;
      viExpect(dateChangedSpy).toHaveBeenCalledTimes(1);
      expect(
        isSameDate(
          dateChangedSpy.mock.calls[0][0].detail.selectedDate,
          new Date("2000/12/15"),
        ),
      ).to.equal(true);
    });
    ...
    describe("Enabled Dates", () => {
      ...
      it('does not prevent initializing "centralDate" from "selectedDate" when today is disabled', async () => {
        vi.useFakeTimers();
        vi.setSystemTime(new Date("2019/06/03"));

        const el = await fixture(html`
          <lion-calendar
            .selectedDate="${new Date("2001/01/08")}"
            .disableDates=${
              /** @param {Date} date */ (date) => date.getDate() === 3
            }
          ></lion-calendar>
        `);
        const elObj = new CalendarObject(el);
      ...
      it("requires the user to set an appropriate centralDate even when minDate and maxDate are equal", async () => {
        ...
        expect(isSameDate(elSetting.centralDate, new Date("2019/07/03"))).to.be
          .true;
      });
  ...
  describe("Calendar navigation", () => {
    describe("Title", () => {
      ...
      it("updates the secondary title when the displayed month/year changes", async () => {
        const el = await fixture(
          html`<lion-calendar
            .centralDate="${new Date("2000/12/12")}"
          ></lion-calendar>`,
        );
        const elObj = new CalendarObject(el);
      ...
      describe("Accessibility", () => {
        it('has aria-atomic="true" set on the secondary title', async () => {
          const elObj = new CalendarObject(
            await fixture(html`<lion-calendar></lion-calendar>`),
          );
          expect(elObj.monthHeadingEl?.getAttribute("aria-atomic")).to.equal(
            "true",
          );
        });
      });
    ...
    describe("Navigation", () => {
      describe("finding enabled dates", () => {
        ...
        it('future dates take precedence over past dates when "distance" between dates is equal', async () => {
          vi.useFakeTimers();
          vi.setSystemTime(new Date("2000/12/15"));

          const el = await fixture(html`
            <lion-calendar
              .disableDates="${
                /** @param {Date} d */ (d) => d.getDate() === 15
              }"
            ></lion-calendar>
          `);
        ...
        it("will search 750 days in the past", async () => {
          ...
          const el = await fixture(html`
            <lion-calendar
              .disableDates="${
                /** @param {Date} d */ (d) => d.getFullYear() > 1998
              }"
            ></lion-calendar>
          `);
          el.focusDate(el.findNearestEnabledDate(new Date()));
        ...
        it("will search 750 days in the future", async () => {
          ...
          const el = await fixture(html`
            <lion-calendar
              .disableDates="${
                /** @param {Date} d */ (d) => d.getFullYear() < 2002
              }"
            ></lion-calendar>
          `);
      ...
      describe("Year", () => {
        ...
        it("disables previousYearButton and nextYearButton based on disabled days accordingly", async () => {
          const el = await fixture(html`
            <lion-calendar
              .selectedDate="${new Date("2000/06/12")}"
            ></lion-calendar>
          `);
          const elObj = new CalendarObject(el);
          expect(elObj.activeMonth).to.equal("June");
          expect(elObj.activeYear).to.equal("2000");
          expect(elObj.previousYearButtonEl?.hasAttribute("disabled")).to.equal(
            false,
          );
          expect(elObj.nextYearButtonEl?.hasAttribute("disabled")).to.equal(
            false,
          );
        ...
        it("sets label to correct month previousYearButton and nextYearButton based on disabled days accordingly", async () => {
          ...
          expect(
            elObj.previousYearButtonEl.getAttribute("aria-label"),
          ).to.equal("Previous year, July 1999");
          expect(elObj.nextYearButtonEl?.hasAttribute("disabled")).to.equal(
            false,
          );
      ...
      describe("Month", () => {
        ...
        it("disables previousMonthButton and nextMonthButton based on disabled days accordingly", async () => {
          ...
          expect(elObj.activeYear).to.equal("2000");
          expect(
            elObj.previousMonthButtonEl?.hasAttribute("disabled"),
          ).to.equal(false);
          expect(elObj.nextMonthButtonEl?.hasAttribute("disabled")).to.equal(
            false,
          );
      ...
      describe("Accessibility", () => {
        it("navigate buttons have a aria-label and title attribute with accessible label", async () => {
          ...
          expect(
            elObj.previousMonthButtonEl?.getAttribute("aria-label"),
          ).to.equal("Previous month, November 2000");
          ...
          expect(
            elObj.previousYearButtonEl.getAttribute("aria-label"),
          ).to.equal("Previous year, December 1999");
          expect(elObj.nextYearButtonEl.getAttribute("title")).to.equal(
  ...
  describe("Calendar body (months view)", () => {
    it("renders the days of the week as table headers", async () => {
      ...
      const elObj = new CalendarObject(el);
      expect(
        elObj.weekdayHeaderEls.map((h) => h.textContent?.trim()),
      ).to.deep.equal(["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]);
    });
    ...
    describe("Day view", () => {
      ...
      it('adds "selected" attribute to the selected date', async () => {
        ...
        expect(
          elObj.checkForAllDayObjs(
            /** @param {DayObject} obj */ (obj) =>
              obj.el.hasAttribute("selected"),
            [12],
          ),
        ).to.equal(true);
      ...
      it('adds aria-disabled="true" attribute to disabled dates', async () => {
        ...
        expect(
          elObj.checkForAllDayObjs(
            /** @param {DayObject} d */ (d) =>
              d.el.getAttribute("aria-disabled") === "true",
            [1, 2, 30, 31],
          ),
        ).to.equal(true);
      ...
      describe("Keyboard Navigation", () => {
        it('focused day is reachable via tab (tabindex="0")', async () => {
          ...
          expect(
            elObj.checkForAllDayObjs(
              /** @param {DayObject} d */ (d) =>
                d.buttonEl.getAttribute("tabindex") === "0",
              [12],
            ),
          ).to.equal(true);
        });
        ...
        it('non focused days are not reachable via tab (have tabindex="-1")', async () => {
          ...
          expect(
            elObj.checkForAllDayObjs(
              /** @param {DayObject} d */ (d) =>
                d.buttonEl.getAttribute("tabindex") === "-1",
        ...
        describe("Arrow keys", () => {
          ...
          it("navigates (sets focus) to previous row via [arrow left] key if first item in row", async () => {
            const el = await fixture(html`
              <lion-calendar
                .selectedDate="${new Date("2019/01/06")}"
              ></lion-calendar>
            `);
            const elObj = new CalendarObject(el);
          ...
          it("navigates to next month via [arrow down] key if last row of month", async () => {
            const el = await fixture(html`
              <lion-calendar
                .selectedDate="${new Date("2000/12/30")}"
              ></lion-calendar>
            `);
            const elObj = new CalendarObject(el);
            expect(elObj.activeMonth).to.equal("December");
      ...
      describe("Initial central", () => {
        ...
        it("is nearest to today if no selected date is available and today is disabled", async () => {
          ...
          const calWithDisabled = await fixture(
            html`<lion-calendar
              .disableDates=${
                /** @param {Date} date */ (date) => date.getDate() === 15
              }
            ></lion-calendar>`,
          );
    ...
    describe("Accessibility", () => {
      ...
      it("adds aria-labels to the weekday table headers", async () => {
        ...
        expect(
          elObj.weekdayHeaderEls.map((h) => h.getAttribute("aria-label")),
        ).to.eql([
          "Sunday",
          "Monday",
          "Tuesday",
      ...
      it("renders days for previous and next months", async () => {
        const elObj = new CalendarObject(
          await fixture(html`
            <lion-calendar
              .selectedDate="${new Date("2000/11/12")}"
            ></lion-calendar>
          `),
        );
      ...
      it("renders days for next months in the last month of the year", async () => {
        const elObj = new CalendarObject(
          await fixture(html`
            <lion-calendar
              .selectedDate="${new Date("2020/12/12")}"
            ></lion-calendar>
          `),
        );
      ...
      it("renders days for previous months in the first month of the year", async () => {
        const elObj = new CalendarObject(
          await fixture(html`
            <lion-calendar
              .selectedDate="${new Date("2020/01/12")}"
            ></lion-calendar>
          `),
        );
      ...
      it('sets aria-current="date" to todays button', async () => {
        ...
        expect(elObj.checkForAllDayObjs(hasAriaCurrent, [monthday])).to.equal(
          true,
        );
      });
  ...
  describe("Localization", () => {
    it("supports custom locale with a fallback to a global locale", async () => {
      const el = await fixture(html`
        <lion-calendar
          .selectedDate="${new Date("2019/12/20")}"
        ></lion-calendar>
      `);
      const elObj = new CalendarObject(el);
      expect(elObj.activeMonth).to.equal("December");
    ...
    it("displays the right translations according to locale", async () => {
      ...
      expect(
        elObj.weekdayHeaderEls.map((h) => h.textContent?.trim()),
      ).to.deep.equal(["zo", "ma", "di", "wo", "do", "vr", "za"]);
    });
  });
