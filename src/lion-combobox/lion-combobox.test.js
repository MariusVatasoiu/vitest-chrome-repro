import { chai, describe, expect, it, vi } from "vitest";
 ...
 * @param {string} keys
 */
async function mimicUserTyping(el, keys) {
  const { _inputNode } = getComboboxMembers(el);
  _inputNode.value = "";
  _inputNode.focus();
  if (!keys) {
    _inputNode.dispatchEvent(
      new Event("input", { bubbles: true, composed: true }),
    );
    return;
  }
  await userEvent.type(_inputNode, keys);
}
...
describe("lion-combobox", () => {
    ...
    it("renders when options changed", async () => {
      ...
      class Wrapper extends LitElement {
        ...
        render() {
          return html`<lion-combobox name="foo">
            ${this.entries.map(
              (entry) =>
                html`<lion-option .choiceValue="${entry.id}"
                  >${entry.label}</lion-option
                >`,
            )}
          </lion-combobox>`;
        }
      ...
      await waitUntil(
        () =>
          wrapperElement?.shadowRoot
            ?.querySelector("lion-options")
            ?.querySelectorAll("lion-option").length === entries.length,
      );
      ...
      await waitUntil(
        () =>
          wrapperElement?.shadowRoot
            ?.querySelector("lion-options")
            ?.querySelectorAll("lion-option").length ===
          wrapperElement.entries.length,
      );

      const optionElements =
        wrapperElement.shadowRoot?.querySelectorAll("lion-option");
  ...
  describe("Options visibility", () => {
    it("hides options when text in input node is cleared after typing something by default", async () => {
      ...
      async function performChecks() {
        ...
        await el.updateComplete;
        expect(visibleOptions().length).to.equal(
          4,
          `autocompleteMode is ${el.autocomplete}`,
        );
        // Remove [hard], [c]
    ...
    it("hides all options on reset()", async () => {
      ...
      const options = el.formElements;
      const visibleOptions = () =>
        options.filter((o) => o.style.display !== "none");
    ...
    it("hides listbox on click/enter (when multiple-choice is false)", async () => {
      ...
      await open();
      expect(el.opened).to.be.true;
      const visibleOptions = el.formElements.filter(
  ...
  describe("Values", () => {
    ...
    it("syncs textbox to modelValue", async () => {
      for (const autocompleteMode of ["none", "list", "inline", "both"]) {
        ...
        expect(el.checkedIndex).to.equal(
          0,
          `autocompleteMode is ${autocompleteMode}`,
        );
    ...
    it("allows a value outside of the option list when requireOptionMatch is false", async () => {
      const el = /** @type {LionCombobox} */ (
        await fixture(html`
          <lion-combobox name="foo" .validators=${[new Required()]}>
            <lion-option checked .choiceValue="${"Artichoke"}"
    ...
    it("submits form on [Enter] when listbox is closed", async () => {
      ...
      const combobox = /** @type {LionCombobox} */ (
        el.querySelector('[name="foo"]')
      );
  ...
  describe("Overlay visibility", () => {
    ...
    it('does not flash the menu when _showOverlayCondition returns "false"', async () => {
      ...
      observer.observe(/** @type {Node} */ (dialog), {
        attributeFilter: ["style"],
      });

      const { _inputNode } = getComboboxMembers(el);
      _inputNode.focus();
      await userEvent.keyboard("art");
    ...
    it("has the correct Overlay Config defined", async () => {
      ...
      const equalsOrContainsInput = (
        /** @type {HTMLInputElement|HTMLDivElement} */ invokerNode,
      ) =>
        // @ts-expect-error [allow-protected] in test
      ...
      it('allows to control overlay visibility via "_showOverlayCondition": should not display overlay if currentValue length condition is not fulfilled', async () => {
        class ShowOverlayConditionCombobox extends LionCombobox {
          ...
          _showOverlayCondition(options) {
            return (
              // @ts-ignore
              this.__prevCboxValueNonSelected.length > 3 &&
              super._showOverlayCondition(options)
            );
          }
        }
    ...
    describe("Accessibility", () => {
      it('sets "aria-posinset" and "aria-setsize" on visible entries', async () => {
        ...
        visibleOptions.forEach((o, i) => {
          ...
          expect(o.getAttribute("aria-setsize")).to.equal(
      ...
      it("dropdown has a label", async () => {
        ...
        expect(
          _overlayCtrl.contentWrapperNode.getAttribute("aria-label"),
        ).to.equal("Combobox options popup");
      });
    });
  });
  ...
  describe("Selection display", () => {
    ...
    it("stores internal reference _selectionDisplayNode in LionCombobox", async () => {
      ...
      expect(_selectionDisplayNode).to.equal(
        el.querySelector("[slot=selection-display]"),
      );
    });
    ...
    it('calls "onComboboxElementUpdated(changedProperties)" on "updated" in _selectionDisplayNode', async () => {
      ...
      const spy = vi.spyOn(
        el._selectionDisplayNode,
        "onComboboxElementUpdated",
      );
  ...
  describe("Autocompletion", () => {
    ...
    it("completes chars inside textbox", async () => {
      ...
      await mimicUserTypingAdvanced(el, [
        "Backspace",
        "Backspace",
        "Backspace",
      ]); // Ch
    ...
    describe("Server side completion support", () => {
      ...
      class MyEl extends LitElement {
        ...
        get combobox() {
          return /** @type {LionCombobox} */ (
            this.shadowRoot?.querySelector("#combobox")
          );
        }
      ...
      it('calls "_handleAutocompletion" after externally changing options', async () => {
        const el = /** @type {MyEl} */ (
          await fixture(html`<${wrappingTag}></${wrappingTag}>`)
        );
        await el.combobox.registrationComplete;
        // @ts-ignore [allow-protected] in test
        const spy = vi.spyOn(el.combobox, "_handleAutocompletion");
        el.addOption();
        await el.updateComplete;
        await el.updateComplete;
        expect(spy).toHaveBeenCalledTimes(1);
    ...
    it("highlights first occurrence of matching option", async () => {
      ...
      expect(options[0]).lightDom.to.equal(
        `<span aria-label="Artichoke">Arti<b>c</b>hoke</span>`,
      );
      expect(options[1]).lightDom.to.equal(
        `<span aria-label="Chard"><b>C</b>hard</span>`,
      );
      expect(options[2]).lightDom.to.equal(
        `<span aria-label="Chicory"><b>C</b>hicory</span>`,
      );
      ...
      await el.updateComplete;
      expect(options[0]).lightDom.to.equal(
        `<span aria-label="Artichoke">Arti<b>ch</b>oke</span>`,
      );
      expect(options[1]).lightDom.to.equal(
        `<span aria-label="Chard"><b>Ch</b>ard</span>`,
      );
      expect(options[2]).lightDom.to.equal(
        `<span aria-label="Chicory"><b>Ch</b>icory</span>`,
      );
      ...
      expect(options[1]).lightDom.to.equal(
        `<span aria-label="Chard">Char<b>d</b></span>`,
      );
    ...
    describe("Subclassers", () => {
      ...
      it('allows to override "_onFilterMatch" and "_onFilterUmatch"', async () => {
        ...
        await el.updateComplete;
        expect(options[0]).lightDom.to.equal(
          `<div data-key>Artichoke</div><small>Cardoon</small>`,
        );
        expect(options[1]).lightDom.to.equal(
          `<div data-key><span aria-label="Chard">Char<b>d</b></div></span><small>Beet</small>`,
        );
        expect(options[2]).lightDom.to.equal(
          `<div data-key>Chicory</div><small>Chicory</small>`,
        );
        expect(options[3]).lightDom.to.equal(
          `<div data-key>Victoria Plum</div><small>Prunus domestica</small>`,
        );
    ...
    describe("Accessibility", () => {
      it("synchronizes autocomplete option to textbox", async () => {
        ...
        expect(
          getComboboxMembers(
            /** @type {LionCombobox} */ (el),
          )._inputNode.getAttribute("aria-autocomplete"),
        ).to.equal("both");
      ...
      it("updates aria-activedescendant on textbox node", async () => {
          ...
          await fixture(html`
            <lion-combobox name="foo" autocomplete="none">
              <lion-option .choiceValue="${"Artichoke"}" id="artichoke-option"
                >Artichoke</lion-option
              >
              <lion-option .choiceValue="${"Chard"}" id="chard-option"
                >Chard</lion-option
              >
              <lion-option .choiceValue="${"Chicory"}">Chicory</lion-option>
              <lion-option .choiceValue="${"Victoria Plum"}"
        ...
        expect(
          elProts._activeDescendantOwnerNode.getAttribute(
        ...
        await el2.updateComplete;
        expect(
          el2Prots._activeDescendantOwnerNode.getAttribute(
            "aria-activedescendant",
          ),
        ).to.equal(el2.formElements[1].id);
      ...
      it("adds aria-label to highlighted options", async () => {
        const [el, options] = await fruitFixture({
          autocomplete: "both",
          matchMode: "all",
        });
        await mimicUserTyping(/** @type {LionCombobox} */ (el), "choke");
        await el.updateComplete;
        const labelledElement = options[0].querySelector(
          'span[aria-label="Artichoke"]',
        );
      ...
      it("adds aria-label to highlighted complex options", async () => {
        ...
        await el.updateComplete;
        const labelledElement = options[0].querySelector(
          'span[aria-label=" Artichoke Cardoon "]',
        );
        expect(labelledElement).to.not.be.null;
        expect(labelledElement.innerText).to.equal("Artichoke\nCardoon");
      });
  ...
  describe("Match Mode", () => {
    ...
    describe("Subclassers", () => {
      it("allows for custom matching functions", async () => {
        ...
        await el.updateComplete;
        expect(
          getFilteredOptionValues(/** @type {LionCombobox} */ (el)),
        ).to.eql(["Chicory"]);
  ...
  describe("Disabled state", () => {
    it("does not open overlay or allow input when disabled", async () => {
      ...
      const { _inputNode } = getComboboxMembers(
        /** @type {LionCombobox} */ (el),
      );
    ...
    it("sets aria-disabled and disables focus when combobox is disabled", async () => {
      const [el] = await fruitFixture({ disabled: true });
      const { _comboboxNode, _inputNode } = getComboboxMembers(
        /** @type {LionCombobox} */ (el),
      );
