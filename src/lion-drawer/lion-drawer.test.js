 ...
 */
const fixture = /** @type {(arg: TemplateResult) => Promise<LionDrawer>} */ (
  _fixture
);
...
describe("<lion-drawer>", () => {
  describe("Drawer", () => {
    ...
    it("sets the minimum and maximum height when position=top", async () => {
      ...
      const minHeight =
        getComputedStyle(drawer).getPropertyValue("--min-height");
      const maxHeight =
        getComputedStyle(drawer).getPropertyValue("--max-height");
