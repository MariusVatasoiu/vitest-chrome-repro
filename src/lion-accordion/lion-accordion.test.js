import { describe, expect as viExpect, it, vi } from "vitest";
...
function getAccordionChildren(el) {
  if (el.shadowRoot) {
    ...
    return slot && slot instanceof HTMLSlotElement
...
function getInvokers(el) {
  return getAccordionChildren(el).filter((child) =>
    child.classList.contains("invoker"),
  );
}
...
describe("<lion-accordion>", () => {
  describe("Accordion", () => {
    ...
    it("can programmatically set expanded", async () => {
      ...
      expect(
        Array.from(getAccordionChildren(el)).find(
          (child) =>
            child.className === "invoker" && child.hasAttribute("expanded"),
        )?.textContent,
      ).to.equal("invoker 2");
    ...
    it("supports [exclusive] attribute, allowing one collapsible to be open at a time", async () => {
      ...
      const invokerButtons = Array.from(getInvokers(el)).map(
        (invokerHeadingEl) =>
          /** @type {HTMLButtonElement} */ (invokerHeadingEl.firstElementChild),
      );
    ...
    it('sends event "expanded-changed" for every expanded state change', async () => {
      ...
      el.expanded = [1];
      viExpect(spy).toHaveBeenCalledTimes(1);
    });
    ...
    it("logs warning if unequal amount of invokers and contents", async () => {
      ...
      `);
      viExpect(warnSpy).toHaveBeenCalledTimes(1);
      viExpect(warnSpy).toHaveBeenCalledWith(
    ...
    it('does not select any elements with slot="invoker" and slot="content" inside slotted elements', async () => {
      ...
      viExpect(warnSpy).not.toHaveBeenCalled();
  ...
  describe("Accordion navigation", () => {
    ...
    it("can programmatically set focusedIndex", async () => {
      ...
      expect(
        Array.from(getInvokers(el)).find((child) =>
          child.firstElementChild?.hasAttribute("focused"),
        )?.textContent,
      ).to.equal("invoker 2");
  ...
  describe("Content distribution", () => {
    it("should work with append children", async () => {
      ...
      expect(
        Array.from(getContents(el)).find((child) =>
          child.hasAttribute("expanded"),
        )?.textContent,
      ).to.equal("content 5");
    });
    ...
    it("should add order style property to each invoker and content", async () => {
      ...
      invokers.forEach((invoker, index) => {
        ...
        expect(invoker.style.getPropertyValue("order")).to.equal(
          `${index + 1}`,
        );
        expect(content.style.getPropertyValue("order")).to.equal(
          `${index + 1}`,
        );
      });
    });
  ...
  describe("Accessibility", () => {
    it("does not make contents focusable", async () => {
      ...
      expect(
        Array.from(getAccordionChildren(el)).find((child) =>
          child.classList.contains("content"),
        ),
    ...
    describe("Invokers", () => {
      it("links ids of content items to invoker first child via [aria-controls]", async () => {
        ...
        const contents = getContents(el);
        expect(
          invokers[0].firstElementChild?.getAttribute("aria-controls"),
        ).to.equal(contents[0].id);
        expect(
          invokers[1].firstElementChild?.getAttribute("aria-controls"),
        ).to.equal(contents[1].id);
      });
      ...
      it('adds aria-expanded="false" to invoker when its content is not expanded', async () => {
        ...
        expect(
          Array.from(getInvokers(el))[0]?.firstElementChild,
    ...
    describe("Contents", () => {
      it("adds aria-labelledby referring to invoker ids", async () => {
        ...
        const contents = getContents(el);
        const invokers = getInvokers(el);
        expect(contents[0]).to.have.attribute(
          "aria-labelledby",
          invokers[0].firstElementChild?.id,
        );
        expect(contents[1]).to.have.attribute(
          "aria-labelledby",
          invokers[1].firstElementChild?.id,
        );
      });
    });
  });
