...
describe("<lion-input-amount>", () => {
  ...
  it("sets correct amount of decimals", async () => {
    const el = /** @type {LionInputAmount} */ (
      await fixture(
        html`<lion-input-amount .modelValue=${100.123}></lion-input-amount>`,
      )
    );
  ...
  it('formats with locale when formatOptions.mode is "user-edited" and value has three decimal places', async () => {
    ...
    const parserSpy = vi.spyOn(el, "parser");
    const formatterSpy = vi.spyOn(el, "formatter");
    ...
    expect(
      parserSpy.mock.calls[parserSpy.mock.calls.length - 1]?.[1]?.mode,
    ).to.equal("user-edited");
    expect(
      formatterSpy.mock.calls[formatterSpy.mock.calls.length - 1]?.[1]?.mode,
    ).to.equal("user-edited");
  ...
  it("shows no currency by default", async () => {
    ...
    expect(Array.from(el.children).find((child) => child.slot === "after")).to
      .be.undefined;
  });
  ...
  it("displays currency if provided", async () => {
    ...
    expect(
      /** @type {HTMLElement[]} */ (Array.from(el.children)).find(
  ...
  it("adds the currency label when currency switches from undefined to EUR", async () => {
    ...
    el.currency = "EUR";
    await el.updateComplete;
    const currLabel = /** @type {HTMLElement[]} */ (
      Array.from(el.children)
    ).find((child) => child.slot === "after");
  ...
  it("sets currency label on the after element", async () => {
    ...
    const mySlotLabel = /** @type {HTMLElement[]} */ (
      Array.from(el.children)
    ).find((child) => child.slot === "after");
  ...
  it("can override the currency label slot", async () => {
    ...
    const el = await fixture(
      html`<my-input-amount currency="EUR"></my-input-amount>`,
    );
    expect(
      /** @type {HTMLElement[]} */ (Array.from(el.children)).find(
        (child) => child.slot === "suffix",
      )?.innerText,
    ).to.equal("EUR");
  });
